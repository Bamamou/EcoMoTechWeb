...
Write-Host "SUCCESS: Build verification passed! Found $fileCount files." -ForegroundColor Green
...
function Upload-Directory {
    ...
    Get-ChildItem -Path $LocalPath -Recurse -File | ForEach-Object {
        ...
        try {
            ...
            Write-Host "SUCCESS: Uploaded $relativePath" -ForegroundColor Gray
            $uploaded++
...
try {
    ...
    if ($result.Failed -gt 0) {
        ...
        Write-Host "NOTE: Some files may have failed due to directory structure. This is often normal." -ForegroundColor Yellow
    }
    ...
    Write-Host "Try running the FTP test script first: .\test-ftp-connection.ps1" -ForegroundColor Yellow
}

Write-Host "`nFor future deployments, you can run:" -ForegroundColor Cyan
Write-Host ".\deploy-manual-fixed.ps1 -Username 'afrixajs' -Password 'yourpassword'" -ForegroundColor White